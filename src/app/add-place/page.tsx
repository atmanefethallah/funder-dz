...
const LocationPickerMap = dynamic(
  () => import("@/components/LocationPickerMap"),
  {
    ssr: false,
    loading: () => (
      <div className="h-[300px] w-full bg-gray-100 animate-pulse rounded-2xl flex items-center justify-center font-bold text-gray-400">
        جاري تحميل خريطة تحديد الموقع... 🗺️
      </div>
    ),
  },
);
...
export default function AddPlacePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (latitude === null || longitude === null) {
      setIsSuccess(false);
      setMessage(
        "⚠️ يرجى تحديد موقع المعلم على الخريطة، لصق رابط Google Maps، أو إدخال الإحداثيات يدوياً.",
      );
      setLoading(false);
      return;
    }
    ...
    const validRoomTypes = roomTypes
      ...
      .filter(
        (room) => room.name && Number.isFinite(room.price) && room.price >= 0,
      );
    ...
    formData.append(
      "eventEndsAt",
      effectiveIsEvent ? new Date(eventEndsAt).toISOString() : "",
    );
    formData.append(
      "roomTypes",
      JSON.stringify(category === "فندق" ? validRoomTypes : []),
    );
  ...
  if (status === "PENDING") {
    return (
      <div
        className="max-w-md mx-auto mt-20 p-8 text-center bg-white border border-gray-100 rounded-3xl shadow-xl animate-fade-in"
        dir="rtl"
      >
        ...
        <h2 className="text-2xl font-black text-gray-900 mb-2">
          وثائقك قيد المراجعة الفنية ⏳
        </h2>
        <p className="text-gray-500 text-sm leading-relaxed mb-6">
          لقد استلمنا نسخة من سجلّك التجاري وبطاقة تعريفك الوطنية بنجاح. تقوم
          الإدارة حالياً بمراجعة البيانات لتفعيل ميزة إضافة المعالم لحسابك.
        </p>
        <button
          onClick={() => router.push("/profile")}
          className="w-full bg-gray-900 text-white font-bold py-3 rounded-xl hover:bg-gray-800 transition text-sm"
        >
          الذهاب للملف الشخصي
        </button>
      </div>
        ...
        <div className="bg-white border rounded-3xl p-8 shadow-md">
          <div className="mb-6 text-center border-b pb-5">
            ...
            <h1 className="text-2xl font-black text-gray-900">
              تفعيل حساب الشريك مطلوب 🛡️
            </h1>
            <p className="text-xs text-gray-400 mt-1">
              تنفيذاً للقوانين؛ يتطلب تفعيل ميزة النشر رفع الوثائق القانونية
              التالية للمنشأة
            </p>
          </div>
          ...
          <form onSubmit={handleVerifySubmit} className="space-y-6">
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-2">
                1. نسخة من السجل التجاري المعتمَد (صورة أو PDF):
              </label>
              <label className="flex flex-col items-center justify-center border-2 border-dashed border-gray-300 rounded-2xl p-5 bg-gray-50 hover:bg-gray-100/70 transition cursor-pointer relative">
                ...
                <span className="text-xs text-gray-500 font-bold text-center">
                  {registryFile
                    ? registryFile.name
                    : "اضغط هنا لاختيار الملف (الحد الأقصى 5MB)"}
                </span>
                <input
                  type="file"
                  accept="image/*,.pdf"
                  onChange={(e) => handleFileChange(e, setRegistryFile)}
                  className="hidden"
                  required
                />
              </label>
            ...
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-2">
                2. نسخة من بطاقة التعريف الوطنية (صورة أو PDF):
              </label>
              <label className="flex flex-col items-center justify-center border-2 border-dashed border-gray-300 rounded-2xl p-5 bg-gray-50 hover:bg-gray-100/70 transition cursor-pointer relative">
                <UploadCloud size={28} className="text-gray-400 mb-1" />
                <span className="text-xs text-gray-500 font-bold text-center">
            ...
            <button
              ...
              className="w-full bg-blue-600 text-white font-black py-4 rounded-xl hover:bg-blue-700 transition shadow-md disabled:opacity-70 flex items-center justify-center gap-2 text-sm"
            >
              {uploadLoading ? (
                <Loader2 className="animate-spin" size={20} />
              ) : (
                "إرسال المستندات للتفعيل الفوري 🚀"
              )}
            </button>
          </form>
        </div>
      ...
      <div className="rounded-2xl border bg-white p-8 shadow-sm">
        <div className="mb-4 flex items-center gap-2 bg-green-50 text-green-700 p-3 rounded-xl border border-green-200">
          ...
          <span className="text-xs font-bold">
            حسابك مفعل وموثق قانونياً. يمكنك الآن إضافة المعالم.
          </span>
        </div>
        ...
        {message && (
          <div
            className={`mb-6 flex items-center gap-2 rounded-lg p-4 font-bold ${isSuccess ? "bg-green-50 text-green-700 border border-green-200" : "bg-red-50 text-red-700 border border-red-200"}`}
          >
            {message}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          <div>
            <label className="mb-2 block text-sm font-medium text-gray-700">
              اسم المعلم أو الفعالية
            </label>
            <input
              type="text"
              value={name}
              onChange={(e) => setName(e.target.value)}
              placeholder="مثال: غابة بورحمة، مسرح مستغانم..."
          ...
          <div>
            ...
            <label className="flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-gray-300 bg-gray-50 p-6 transition hover:bg-gray-100 hover:border-blue-400">
              ...
              <span className="text-sm text-gray-500 font-medium">
                {image ? image.name : "اضغط هنا لاختيار صورة من جهازك"}
              </span>
              <input
                type="file"
                accept="image/*"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  setCategory(value);
                  if (value === "فندق")
                    router.push("/partner-dashboard/properties/new");
                  if (value === "فعالية")
                    router.push("/partner-dashboard/events/new");
                }}
                className="w-full rounded-lg border p-3 outline-none focus:border-blue-500 bg-white"
              >
            ...
            <div>
              ...
              <input
                ...
                placeholder="اتركه فارغاً مجانياً"
                className="w-full rounded-lg border p-3 outline-none focus:border-blue-500"
              />
            </div>
          </div>
          ...
          <div className="rounded-xl border border-pink-100 bg-pink-50/60 p-4 space-y-3">
            <label className="flex items-center gap-2 text-sm font-bold text-gray-800 cursor-pointer">
              <input
                type="checkbox"
                checked={isEvent || category === "فعالية"}
                disabled={category === "فعالية"}
            ...
            {(isEvent || category === "فعالية") && (
              <div>
                <label className="mb-2 flex items-center gap-1 text-xs font-bold text-pink-800">
                  <CalendarDays size={15} /> تاريخ ووقت انتهاء الفعالية
                </label>
                <input
                  type="datetime-local"
                  value={eventEndsAt}
                  onChange={(e) => setEventEndsAt(e.target.value)}
                  className="w-full rounded-lg border border-pink-200 bg-white p-3 outline-none focus:border-pink-500"
                  required
                />
                <p className="mt-1 text-[11px] text-pink-700">
                  بعد هذا الموعد ستختفي الفعالية تلقائياً من صفحات الاستكشاف
                  والحجز.
                </p>
              </div>
            )}
          ...
          {category === "فندق" && (
            <div className="rounded-xl border border-cyan-100 bg-cyan-50/60 p-4 space-y-3">
              <div className="flex items-center justify-between gap-3">
                <label className="flex items-center gap-1 text-sm font-bold text-cyan-900">
                  <BedDouble size={17} /> أنواع الغرف وأسعارها
                </label>
                <button
                  type="button"
                  onClick={() =>
                    setRoomTypes((items) => [...items, { name: "", price: "" }])
                  }
              ...
              {roomTypes.map((room, index) => (
                ...
                >
                  <input
                    ...
                    placeholder="مثال: جناح عائلي"
                    className="rounded-lg border border-cyan-200 bg-white p-2.5 text-sm outline-none focus:border-cyan-500"
                  />
                  <input
                    type="number"
                    min="0"
          ...
          <div className="flex flex-col gap-2">
            <label className="text-sm font-bold text-gray-700">
              رابط صورة 360 درجة (الجولة الافتراضية) 🕶️{" "}
            ...
            <p className="text-xs text-gray-500">
              يجب أن يكون الرابط لصورة بانورامية بصيغة JPG أو PNG تدعم العرض
              المحيطي (Equirectangular).
            </p>
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="اكتب نبذة عن المكان تجذب السياح..."
              rows={4}
              className="w-full rounded-lg border p-3 outline-none focus:border-blue-500"
            />
          </div>
          ...
          <div className="mt-2 rounded-xl border border-gray-200 bg-gray-50 p-4">
            <label className="mb-3 block text-sm font-bold text-gray-700 flex items-center gap-2">
              <MapPin size={18} className="text-blue-600" /> حدد موقع المعلم
              الجغرافي <span className="text-red-500">*</span>
            </label>
            <p className="text-xs text-gray-500 mb-3">
              انقر على الخريطة لتثبيت الدبوس، أو أدخل الإحداثيات يدوياً من خرائط
              جوجل للحصول على دقة متناهية.
            </p>

            {/* 🔗 رابط Google Maps المباشر */}
            <div className="mb-4">
              <label className="mb-1 flex items-center gap-1 text-[11px] font-bold text-gray-600">
                <Link2 size={14} /> رابط المكان من Google Maps
              </label>
              <input
                type="url"
                value={mapsLink}
                onChange={(e) => handleMapsLinkChange(e.target.value)}
                placeholder="https://maps.google.com/... أو رابط مشاركة المكان"
                className="w-full rounded-lg border border-gray-300 p-3 text-sm outline-none focus:border-blue-500 text-left"
                dir="ltr"
              />
              <p className="mt-1 text-[10px] text-gray-500">
                إذا كان الرابط المختصر لا يحتوي الإحداثيات، افتح المكان في
                المتصفح وانسخ الرابط الكامل.
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <input
                  ...
                  value={latitude || ""}
                  onChange={(e) =>
                    handleManualCoordinateChange("lat", e.target.value)
                  }
                  placeholder="مثال: 35.931"
                  className="w-full rounded-lg border border-gray-300 p-2 text-sm outline-none focus:border-blue-500 text-left"
                  dir="ltr"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  value={longitude || ""}
                  onChange={(e) =>
                    handleManualCoordinateChange("lng", e.target.value)
                  }
            ...
            {latitude && longitude && (
              <div className="mt-3 inline-flex items-center gap-2 rounded-lg bg-green-100 px-3 py-2 text-xs font-bold text-green-800 border border-green-200">
                ✅ تم تحديد الموقع: {latitude.toFixed(5)},{" "}
                {longitude.toFixed(5)}
              </div>
            )}
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="mt-4 flex w-full items-center justify-center rounded-lg bg-blue-600 p-4 font-bold text-white transition hover:bg-blue-700 disabled:opacity-70 shadow-md gap-2"
          >
            {loading ? (
              <Loader2 className="animate-spin" size={24} />
            ) : (
              "نشر المعلم في المنصة 🚀"
            )}
          </button>
        </form>
      </div>
