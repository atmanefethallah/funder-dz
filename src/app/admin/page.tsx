...
export default function AdminDashboard() {
  ...
  const handleAction = async (
    ...
    const okAction = await confirmDialog({
      ...
      message:
        action === "APPROVE"
          ? "سيتم شحن رصيد المستخدم بهذا المبلغ فوراً."
          : "سيتم رفض طلب الشحن هذا.",
  ...
  if (loading) {
    return (
      <div className="flex h-[70vh] flex-col gap-4 items-center justify-center">
        ...
        <p className="font-bold text-gray-500">جاري تحميل بيانات المنصة...</p>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8">
      ...
      <div className="flex flex-wrap gap-3 mb-10">
        <Link
          href="/admin/vouchers"
          className="flex items-center gap-2 bg-white border shadow-sm hover:shadow-md hover:-translate-y-0.5 transition rounded-xl px-4 py-2.5 font-bold text-sm text-gray-700"
        >
        ...
        <Link
          ...
          className="flex items-center gap-2 bg-white border shadow-sm hover:shadow-md hover:-translate-y-0.5 transition rounded-xl px-4 py-2.5 font-bold text-sm text-gray-700"
        >
          <UserCheck size={18} className="text-emerald-600" /> طلبات اعتماد
          الشركاء
        </Link>
        <Link
          ...
          className="flex items-center gap-2 bg-white border shadow-sm hover:shadow-md hover:-translate-y-0.5 transition rounded-xl px-4 py-2.5 font-bold text-sm text-gray-700"
        >
          <Megaphone size={18} className="text-fuchsia-600" /> مراجعة حملات
          Promote
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
        ...
        <div className="bg-white rounded-2xl p-6 border shadow-sm flex flex-col justify-center items-center text-center transition hover:-translate-y-1">
          <div className="bg-amber-50 p-4 rounded-full text-amber-600 mb-4">
            <MapIcon size={32} />
          </div>
          <p className="text-sm text-gray-500 font-bold mb-1">
            المعالم المسجلة
        ...
        <div className="bg-gradient-to-br from-green-500 to-green-700 text-white rounded-2xl p-6 border border-green-600 shadow-lg flex flex-col justify-center items-center text-center transition hover:-translate-y-1 relative overflow-hidden">
          ...
          <p className="text-sm text-green-100 font-bold mb-2">
            إجمالي حركة الأموال
          </p>
        ...
        {requests.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-gray-200 py-16 text-gray-400">
            ...
            <p className="text-sm mt-2 text-gray-400">
              لقد قمت بمعالجة كافة الطلبات بنجاح ✨
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {requests.map((req) => (
              <div
                key={req.id}
                className="flex flex-col overflow-hidden rounded-xl border border-gray-200 shadow-sm transition hover:shadow-md bg-white"
              >
                {/* صورة الوصل */}
                <div className="relative h-48 bg-gray-100 group">
                  ...
                  <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm">
                    <a
                      ...
                      className="flex items-center gap-2 rounded-lg bg-white px-4 py-2 text-sm font-bold text-gray-900 hover:scale-105 transition"
                    >
                ...
                <div className="p-5 flex flex-col flex-1">
                  ...
                  <div className="mb-6 bg-blue-50/50 rounded-lg p-4 text-center border border-blue-100 mt-auto">
                    ...
                    <p className="text-2xl font-black text-blue-700" dir="ltr">
                      {req.amount} د.ج
                    </p>
                  </div>
        ...
        <form onSubmit={handleSendNotification} className="flex flex-col gap-4">
          <div>
            ...
            <input
              name="title"
              required
              placeholder="مثال: تخفيض 50% على تذاكر الغابة!"
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full rounded-xl border border-gray-300 p-3 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition min-h-[120px] resize-none"
            />
          </div>
          ...
          <div>
            ...
            <input
              name="link"
              placeholder="مثال: /explore"
              className="w-full rounded-xl border border-gray-300 p-3 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition text-left"
              dir="ltr"
            />
            <p className="text-xs text-gray-500 mt-1">
              عندما يضغط السائح على الإشعار سينتقل لهذا الرابط.
            </p>
          </div>
